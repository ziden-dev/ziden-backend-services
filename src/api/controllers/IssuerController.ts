...
export class IssuerController {
    ...
    public async findIssuerOperators(req: Request, res: Response) {
        try {
            if (!req.params.issuerId) throw new BadRequestError('Missing issuerId in request parmas');

            res.send({
                'operators': await this.operatorService.findByIssuer(req.params.issuerId) 
            });
        } catch (error: any) {
            logger.error(error);
            res.status(error.httpCode ?? 500).send(error);
    ...
    public async addOperator(req: Request, res: Response) {
        try {
            if (!req.params.issuerId) throw new BadRequestError('Missing operatorId in request parmas');
            if (!req.body.operatorId) throw new BadRequestError('Missing operatorId in request body');

            const newOperator = await this.operatorService.addOperatorByIssuer(
                req.body.operatorId,
                req.params.issuerId
            )
    ...
    public async removeOperator(req: Request, res: Response) {
        try {
            ...
            const operator = await this.operatorService.removeOperatorByIssuer(
